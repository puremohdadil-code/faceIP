...
def render_with_ruby(snapshot: dict[str, Any], ruby_script: Path | None) -> str | None:
    if not ruby_script or not ruby_script.exists():
        return None
    try:
        result = subprocess.run(
            ["ruby", str(ruby_script)],
            input=json.dumps(snapshot), text=True, capture_output=True, timeout=10, check=False,
        )
    except OSError:
        return None
